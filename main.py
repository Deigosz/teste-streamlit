...
def _load_fichas_from_disk():
    if FICHA_FILE.exists() and FICHA_FILE.stat().st_size > 0:
        try:
            ...
            st.warning(
                "⚠️ Erro ao decodificar ficha_contagem.json. O arquivo pode estar corrompido. Usando dados padrão.",
                icon="⚠️")
    ...
    try:
        if os.path.exists(caminho_produtos):
            ...
            mock_data = {
                ...
                'descricao': ['Leite Integral UHT 1L', 'Creme de Leite 200g', 'Queijo Mussarela 5kg',
                              'Manteiga Extra 500g', 'Iogurte Natural 1L', 'Bebida Láctea TP 1L'],
        ...
        st.error(
            f"❌ Erro ao carregar dados: Verifique se o arquivo CSV '{caminho_produtos}' está formatado corretamente. Detalhes: {e}")
        return pd.DataFrame()
    except Exception as e:
...
def ficha_management():
    ...
    with col1:
        if fichas_lista:
            current_index = fichas_lista.index(
                st.session_state.ficha_atual) if st.session_state.ficha_atual in fichas_lista else 0
            ficha_selecionada = st.selectbox("Selecione uma ficha:", fichas_lista, index=current_index,
                                             label_visibility="collapsed", key="selectbox_fichas")
        else:
    ...
    if ficha_selecionada:
        if ficha_selecionada != st.session_state.get('ficha_atual'):
            ...
            st.toast(
                f"Ficha '{ficha_selecionada}' carregada com {len(st.session_state.contagens_registradas)} registros!",
                icon="📄", duration=3)
            st.rerun()
...
def contagem_local_especifico():
    ...
    with col_barracao:
        barracao_selecionado = st.radio(
            "Selecione o Barracão:",
            options=lista_barracoes,
            horizontal=True,   # deixa lado a lado (estilo botões)
            key="barracao_selecionavel"
        )
    ...
    # 2. Seu selectbox normal
    with col_rua_inicial:
        rua_inicial = st.selectbox(
            # ... seu código de selectbox aqui
            "Rua Inicial:",
            lista_ruas,
        ...
        with col_contagem_rapida:
            ...
            if st.session_state.contagens_registradas:
                ...
                with col_tabela:
                    st.dataframe(
                        df_registros,
                        column_order=["Hora", "SKU", "Descrição", "Barracão", "Rua Inicial", "Rua Final", "Pallets",
                                      "Caixas Soltas"],
                        column_config={
...
if __name__ == "__main__":
    ...
    if st.session_state.fichas_lista:
        ...
        if 'contagens_registradas' not in st.session_state or st.session_state.contagens_registradas != \
                ficha_encontrada["data"]:
            st.session_state.contagens_registradas = ficha_encontrada["data"] if ficha_encontrada else []
